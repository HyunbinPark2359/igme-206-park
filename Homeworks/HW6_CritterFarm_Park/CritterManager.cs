...
namespace HW6_CritterFarm
{
    ...
    /// Manage the critters and a save file
    /// </summary>
    class CritterManager
    {
        ...
        public void LoadCrittersFromFile()
        {
            ...
            try
            {
                ...
                // Call SetupCritters if there are no saved critters in a file
                if ((line = reader.ReadLine()) == null || line == "No critters saved.")
                {
                    SmartConsole.PrintError("There are no saved critters in a file.\n\n" +
                    "You'll need to set up a new critter farm.\n");
                    SetupCritters();
                }
                // Loop through the file if file data exists
                else if (line != null)
                {
                    do
                    {
                        // Split the line to get the critter parameters
                        string[] data = line.Split("|");

                        // Define necessary variables to check if the line is defective
                        CritterType type = CritterType.Cat;
                        int result = -1;

                        // Determine the critter type and
                        // Print a warning message if the type is defective and skip that line
                        if (!Enum.TryParse<CritterType>(data[0], true, out type)
                            || !Enum.IsDefined(typeof(CritterType), type))
                        {
                            SmartConsole.PrintWarning($"{data[0]}'s aren't supported yet. Skipping this line: {line}\n");
                        }
                        // Print a warning message if the data is defective and skip that line
                        else if (!int.TryParse(data[2], out result) || !int.TryParse(data[3], out result))
                        {
                            SmartConsole.PrintWarning($"Corrupt data. Skipping this line: {line}");
                        }
                        // Add a critter to the list of critters
                        else
                        {
                            switch (type)
                            {
                                case CritterType.Cat:
                                    critterList.Add(new Cat(data[1], int.Parse(data[2]), int.Parse(data[3])));
                                    break;

                                case CritterType.Dog:
                                    critterList.Add(new Dog(data[1], int.Parse(data[2]), int.Parse(data[3])));
                                    break;

                                case CritterType.Horse:
                                    critterList.Add(new Horse(data[1], int.Parse(data[2]), int.Parse(data[3])));
                                    break;

                                default:
                                    // shouldn't happen
                                    SmartConsole.PrintError(String.Format("Not sure how to create a critter with a type of {0}", type));
                                    break;
                            }
                        }
                    }
        ...
        public void SaveCrittersToFile()
        {
            ...
            try
            {
                ...
                // If critters exist, Write a line to the file for every critter in the list
                if (critterList != null)
                {
                    foreach (Critter c in critterList)
                    {
                        writer.WriteLine($"{c.Type}|{c.Name}|{c.Hunger}|{c.Boredom}");
                    }
                }
                // If there is no critter to save, write "No critters saved."
                else
                {
                    writer.WriteLine("No critters saved.");
                }
        ...
        public void TimePassing()
        {
            foreach (Critter c in critterList)
            {
                ...
                // Call CauseMischeif on any cats 25% of the time
                int randomNumber = rng.Next(100);
                if (randomNumber < 25 && c is Cat)
                {
                    Cat car = (Cat)c;
                    car.CauseMischief();
                }
                // Call RunAround on any dogs 25% of the time
                else if (randomNumber < 25 && c is Dog)
                {
                    Dog d = (Dog)c;
                    d.RunAround();
                }
